...
function loadServer() {
	...
	$('#shDefault').append(
			'<li><a onclick="addrClick(\'' + defaultServer + '\')">'
					+ defaultServer + '</a></li>');
	seedServerHistory();
	...
	// Merge tables
		...
		for (i = 1; i < numColumns; i++) {
			for ( var j = 0; j < numRows; j++) {
				// Check if row exists
				matchingRows = dataQueries[queryIndex].queryResults
						.getFilteredRows([ {
							column : 0,
				...
				if (matchingRows.length < 1) {
					...
					dataQueries[queryIndex].queryResults.setValue(newRow, 0,
							table.getValue(j, 0));
					dataQueries[queryIndex].queryResults.setValue(newRow, i,
							table.getValue(j, i));
				}
				...
				// Else update matching rows
					...
					for ( var l = 0; l < matchingRows.length; l++) {
						dataQueries[queryIndex].queryResults.setValue(
								matchingRows[l], i, table.getValue(j, i));
					}
				}
			}
		}
	...
	if (dataArray.length >= 1) {
		...
		for (i = 1; i < dataArray.length; i++) {
			...
			for (j = 1; j < dataArray[i].getNumberOfColumns(); j++) {
				newColumn = mergedTable.addColumn(
						dataArray[i].getColumnType(j), dataArray[i]
								.getColumnLabel(j));
				for ( var k = 0; k < dataArray[i].getNumberOfRows(); k++) {
					...
					if (matchingRows.length < 1) {
						...
						mergedTable.setValue(newRow, 0, dataArray[i].getValue(
								k, 0));
						mergedTable.setValue(newRow, newColumn, dataArray[i]
								.getValue(k, j));
					} else {
						for (l = 0; l < matchingRows.length; l++) {
							mergedTable.setValue(matchingRows[l], newColumn,
									dataArray[i].getValue(k, j));
						}
					}
				}
			}
...
function splitQuery(server, source, dataType, startTime, endTime, interval) {
	...
	while(tempTime < endTime) {
	    query = new google.visualization.Query(server +'wattdepot/admin/depository/power/measurements/gviz/?sensor='+ source +'&start='+wdClient.getTimestampFromDate(startTime)+'&end='+wdClient.getTimestampFromDate(tempTime)+'&samplingInterval='+interval);
	...
	query = new google.visualization.Query(server
			+ 'wattdepot/admin/depository/' + dataType + '/values/gviz/?sensor='
			+ source + '&start=' + wdClient.getTimestampFromDate(startTime)
			+ '&end=' + wdClient.getTimestampFromDate(endTime) + '&interval='
			+ interval);
	query.setTimeout(queryTimeOut);
	returnArray.push(query);
	return returnArray;
	...
	for ( var loopIndex = 0; loopIndex < activeIndex.length; loopIndex++) {
		...
		if (activeIndex[loopIndex].sourceSelected) {
			...
			if (findSource(source) != -1) {
				if ($('#sourceSelect' + formIndex + "_chzn")
						.hasClass('invalid')) {
					$('#sourceSelect' + formIndex + "_chzn").parent().tooltip(
							'hide');
					$('#sourceSelect' + formIndex + "_chzn").unwrap();
					$('#sourceSelect' + formIndex + "_chzn").removeClass(
							'invalid');
				}
			}
		...
		case "energy":
			...
			if (!error) {
				dataQueries.push({
					'source' : source,
					'numReturned' : 0,
					'queries' : splitQuery(server, source, dataType, startTime,
							endTime, interval),
					'queryResults' : null
				});
			}
...
function sourceChange(formIndex, dataTypeGenerator, selectedDataType) {
	...
	if (selectedDataType == "") {
		...
		wdClient.getSourceLatestSensorData(
				$('#sourceSelect' + formIndex).val(), function(data) {
...
function insertVisualizerFormHTML(formIndex) {
	...
	.append(
		'<div class="row-fluid form" id="form' + formIndex + '">'
			+ '<div class="row-fluid">'
			+ '<div id="sourceDiv' + formIndex + '" class="span2 control-group">'
			+ '<div class="span2"><a href="#" rel="tooltip" data-placement="right" title="Show / Hide the source.">'
			+ '<input type="checkbox" class="span11" id="show' + formIndex + '" /></a></div>'
			+ '<div class="span10">'
			+ '<select id="sourceSelect' + formIndex + '" class="span12 chzn-select" data-placeholder="Choose a source...">'
			+ '<option value="" selected></option></select></div></div>'
			+ '<div id="dataTypeDiv' + formIndex + '" class="span2 control-group">'
			+ '<select class="span12" id="dataType' + formIndex + '"></select></div>'
			+ '<div class="span7" id="timeDiv' + formIndex + '">'
			+ '<div class="span5 control-group">'
			+ '<div class="input-append bootstrap-timepicker-component span5">'
			+ '<input type="text" id="startTimePicker' + formIndex + '" class="timepicker-default input-small">'
			+ '<span id="startTimePickerSpan' + formIndex + '" class="add-on"><i class="icon-time"></i></span></div>'
			+ '<div class="span5">'
			+ '<input class="span12" type="text" value="' + (currentDate.getMonth() + 1) + '/' + (currentDate.getDate()) + '/' + currentDate.getFullYear()
			+ '" id="startDatepicker' + formIndex + '"></div>'
			+ '<div class="span12 boundaryData"><div class="span5">Earliest Sensor:</div>'
			+ '<div class="span6" id="earliestSensor' + formIndex + '"></div></div></div>'
			+ '<div class="span5 control-group"><div class="input-append bootstrap-timepicker-component span5">'
			+ '<input type="text" id="endTimePicker' + formIndex + '" class="timepicker-default input-small">'
			+ '<span id="endTimePickerSpan' + formIndex + '" class="add-on"><i class="icon-time"></i></span></div>'
			+ '<div class="span5"><input class="span12" type="text" value="' + (currentDate.getMonth() + 1) + '/' + currentDate.getDate() + '/' + currentDate.getFullYear()
			+ '" id="endDatepicker' + formIndex + '"></div>'
			+ '<div class="span2"><label class="checkbox"><input type="checkbox" id="endTimeNow' + formIndex + '" value="now">Now</label></div>'
			+ '<div class="span12 boundaryData"><div class="span5">Latest Sensor:</div>'
			+ '<div id="latestSensor' + formIndex + '" class="span6"></div></div></div>'
			...
			+ '<div class="span1" id="removeDiv' + formIndex + '">'
			+ '<a href="#" rel="tooltip" data-placement="left" title="Remove the source.">'
			+ '<button id="remove' + formIndex + '" class="btn"><i class="icon-minus-sign"></i></button></a></div></div></div></div>');
...
function addVisualizerForm() {
	if (activeIndex.length == 0) {
		...
		if (activeIndex[activeIndex.length - 1].sourceSelected) {
			createFilledVisualizerForm($('#show' + lastActiveFormIndex).is(
					':checked'), $(
					'#sourceSelect' + lastActiveFormIndex + ' option:selected')
					.val(), $(
					'#dataType' + lastActiveFormIndex + ' option:selected')
					.val(), $('#startTimePicker' + lastActiveFormIndex).val(),
			...
			createFilledVisualizerForm($('#show' + lastActiveFormIndex).is(
					...
					'#endDatepicker' + lastActiveFormIndex).val(), $(
					'#endTimeNow' + lastActiveFormIndex).is(':checked'), $(
					'#interval' + lastActiveFormIndex + ' option:selected')
					.val(), putAndSelectDataTypes);
		}
	}
}
